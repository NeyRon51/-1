class Todo {
  ...
  constructor(rootElement) {
    ...
    this.localStorageKey = 'todo-items-' + this.rootElement.querySelector('.todo__title').textContent.trim().toLowerCase()
  ...
  render() {
    ...
    this.listElement.innerHTML = items.map(({ id, title, isChecked }) => `
      <li class="todo__item todo-item" data-js-todo-item>
        ...
        <button class="todo-item__delete-button" data-js-todo-item-delete-button aria-label="Delete" title="Delete">
    ...
    this.emptyMessageElement.textContent =
      isEmptyFilteredItems ? 'Tasks not found' : isEmptyItems ? 'There are no tasks yet' : ''
  }
...
document.querySelectorAll('[data-js-todo]').forEach(root => new Todo(root))
